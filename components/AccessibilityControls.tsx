...
export default function AccessibilityControls({ locale }: { locale: "ar" | "en" }) {
  ...
  return (
    <>
      {open && (
        <div 
          className="fixed inset-0 z-[65] bg-black/20 backdrop-blur-sm animate-in fade-in duration-300 md:hidden" 
      ...
      >
        ...
        {open && (
          <div 
            className="absolute bottom-16 start-0 w-[min(22rem,calc(100vw-3rem))] max-h-[calc(100vh-12rem)] flex flex-col rounded-[2.5rem] border border-[#E6D7C8]/60 bg-[#F7F2EC]/95 text-[#2C160F] shadow-[0_20px_60px_rgba(44,22,15,0.18)] backdrop-blur-2xl animate-in slide-in-from-bottom-4 fade-in duration-300"
          >
